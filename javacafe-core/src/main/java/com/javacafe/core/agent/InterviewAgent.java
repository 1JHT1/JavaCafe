...
public class InterviewAgent {
    ...
    private Flux<String> runLatte(String sessionId, String userId) {
        String memoryContext = vectorRetrievalTool.retrieve(userId, "Java基础知识");
        // 预置外部知识库检索：为出题与评估提供八股文知识支撑（无知识库时返回空串）
        String knowledgeContext = vectorRetrievalTool.retrieveKnowledge(
                "Java基础知识", com.javacafe.common.constant.BusinessConstants.DEFAULT_TOP_K_RETRIEVAL);
